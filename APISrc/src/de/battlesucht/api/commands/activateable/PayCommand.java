...
public class PayCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                p.sendMessage(Language.prefix + "Bitte benutze: §e/pay <Spieler> <Anzahl>");
                return false;
            }
            if (args.length == 1) {
                p.sendMessage(Language.prefix + "Bitte benutze: §e/pay <Spieler> <Anzahl>");
            ...
            if (args.length == 2) {
                if (args[0].equalsIgnoreCase("*")) {
                    ...
                    if (CoinsAPI.getCoins(p) == Bukkit.getOnlinePlayers().size() * Double.valueOf(args[1]).intValue() || CoinsAPI.getCoins(p) >= Bukkit.getOnlinePlayers().size() * Double.valueOf(args[1]).intValue()) {
                        p.sendMessage(Language.prefix + "Du hast erfolgreich §e" + args[1] + " Coins §7an alle Spieler gegeben!");
                        ...
                        for (Player all : Bukkit.getOnlinePlayers()) {
                            ...
                            all.sendMessage(Language.prefix+ "Du hast von §b" + p.getName() + " §e" + args[1] + " Coins §7erhalten!");
                        }
                        return true;
                    }
                ...
                if (target.isOnline()) {
                    if (CoinsAPI.getCoins(p) == Double.valueOf(args[1]) || CoinsAPI.getCoins(p) >= Double.valueOf(args[1]).intValue()) {
                        p.sendMessage(Language.prefix+ "Du hast erfolgreich §e" + args[1] + " Coins§7 an §b" + args[0] + " §7gezahlt!");
                        target.sendMessage(Language.prefix+ "Du hast von §b" + p.getName() + " §e" + args[1] + " Coins §7erhalten!");
