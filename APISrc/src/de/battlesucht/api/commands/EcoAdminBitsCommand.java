...
public class EcoAdminBitsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(commandSender.hasPermission("battleapi.ecoadminbits")){

            if(strings.length==0) {
                p.sendMessage(Language.prefix + "Bitte verwende: §a/ecoadminbits <show, give, take, reset> <Spieler>  [Anzahl]");
                return false;
                ...
                if(strings[0].equalsIgnoreCase("show")){
                    if(Bukkit.getPlayer(strings[1]).isOnline()==false){
                        p.sendMessage(Language.prefix+"Der Spieler §e"+strings[1]+"§7 hat aktuell "+ BitsAPI.getBits(Bukkit.getOfflinePlayer(strings[1])) + " Bits.");
                        return true;
                    }
                    p.sendMessage(Language.prefix+"Der Spieler §e"+strings[1]+"§7 hat aktuell "+ BitsAPI.getBits(Bukkit.getPlayer(strings[1])) + " Bits.");
                    return true;
                } else if(strings[0].equalsIgnoreCase("reset")){
                    if(Bukkit.getPlayer(strings[1]).isOnline()==false){
                        BitsAPI.resetBits(Bukkit.getOfflinePlayer(strings[1]));
                        p.sendMessage(Language.prefix+"Der Spieler §e"+strings[1]+"§7 hat nun "+ BitsAPI.getBits(Bukkit.getOfflinePlayer(strings[1])) + " Bits.");
                        return true;
                    }
                    BitsAPI.resetBits(Bukkit.getPlayer(strings[1]));
                    p.sendMessage(Language.prefix+"Der Spieler §e"+strings[1]+"§7 hat nun "+ BitsAPI.getBits(Bukkit.getPlayer(strings[1])) + " Bits.");
                    return true;
                }
